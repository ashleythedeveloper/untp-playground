...
const appSg = new aws.ec2.SecurityGroup(`app-sg-${stack}`, {
  vpcId: vpc.vpcId,
  ingress: [
...
const appLBSg = new aws.ec2.SecurityGroup(`app-lb-sg-${stack}`, {
  vpcId: vpc.vpcId,
  ingress: [
    { protocol: "tcp", fromPort: 3000, toPort: 3000, cidrBlocks: ["0.0.0.0/0"] },
...
const targetGroup = new aws.lb.TargetGroup(`app-tg-${stack}`, {
  targetType: "ip",
  vpcId: vpc.vpcId,
  port: 3000,
  protocol: "HTTP",
...
const lb = new awsx.lb.ApplicationLoadBalancer(`app-lb-${stack}`, {
...
const service = new awsx.ecs.FargateService(`app-service-${stack}`, {
  cluster: cluster.arn,
  desiredCount: 1,
  taskDefinitionArgs: {
    container: {
